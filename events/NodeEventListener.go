...
type NodeEventListener interface {
	...
	//OnBootstrapComplete is called when the node completes its bootstrap process which involves connecting to bootstrap nodes and populating its routing table with initial peer information.
	OnBootstrapComplete()
}
